    ...
    const testimonials = [
        {
            ...
            textEN: "FUJI FD delivered exceptional elevator solutions for our commercial complex. Their attention to safety and quality is unmatched.",
            textAR: "قدمت فوجي إف دي حلول مصاعد استثنائية لمجمعنا التجاري. اهتمامهم بالسلامة والجودة لا مثيل له.",
            rating: 5,
        ...
        <section className="bg-gray-50 py-12 sm:py-16 lg:py-24">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-16">
                ...
                <div className="text-center mb-12 sm:mb-16">
                    <h2 className={`text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-fujiBlue mb-4 sm:mb-6 ${isRTL ? 'font-cairo' : ''}`}>
                        {t('Our_Clients')}
                    </h2>
                    <p className={`text-base sm:text-lg text-[#C0392B] max-w-3xl mx-auto leading-relaxed px-4 ${isRTL ? 'font-cairo' : ''}`}>
                        {isRTL 
                        ...
                        <div className="flex transition-transform duration-500 ease-in-out" 
                            ...
                            {Our_Clients_Data.concat(Our_Clients_Data).map((client, index) => (
                                <div key={`${client.id}-${index}`} className="flex-none w-1/2 sm:w-1/3 md:w-1/4 lg:w-1/5 p-4 sm:p-6 lg:p-8">
                                    <div className={`group relative h-20 sm:h-24 md:h-28 lg:h-32 rounded-xl sm:rounded-2xl overflow-hidden transition-all duration-300 ${
                                        ...
                                        <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                            <div className="absolute bottom-2 sm:bottom-4 left-2 sm:left-4 right-2 sm:right-4">
                                                <h4 className={`text-white text-xs sm:text-sm font-semibold ${isRTL ? 'font-cairo text-right' : ''}`}>
                ...
                <div className="bg-white rounded-2xl sm:rounded-3xl shadow-soft border border-gray-200 p-6 sm:p-8 lg:p-12">
                    <div className="text-center mb-8 sm:mb-12">
                        <h3 className={`text-2xl sm:text-3xl lg:text-4xl font-bold text-fujiBlue mb-4 ${isRTL ? 'font-cairo' : ''}`}>
                            {isRTL ? 'ماذا يقول عملاؤنا' : 'What Our Clients Say'}
                        </h3>
                        <p className={`text-[#C0392B] ${isRTL ? 'font-cairo' : ''}`}>
                            ...
                            {testimonials.map((testimonial, index) => (
                                <div key={testimonial.id} className="bg-gray-50 rounded-2xl p-6 hover:shadow-lg transition-all duration-300">
                                    <div className={`text-center ${isRTL ? 'font-cairo' : ''}`}>
                                        ...
                                        {/* Quote */}
                                        <blockquote className={`text-base text-gray-700 leading-relaxed mb-6 italic ${isRTL ? 'text-right' : ''}`}>
                                            "{isRTL ? testimonial.textAR : testimonial.textEN}"
                                        </blockquote>
                ...
                <div className="mt-12 sm:mt-16 grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
                    {[
                        ...
                        <div key={index} className="text-center p-4 sm:p-6 bg-white rounded-xl sm:rounded-2xl shadow-soft">
                            <div className="text-2xl sm:text-3xl lg:text-4xl font-bold text-brandRed mb-2">
                                {isRTL ? stat.numberAR : stat.numberEN}
                            </div>
                            <div className={`text-sm sm:text-base text-[#C0392B] ${isRTL ? 'font-cairo' : ''}`}>
