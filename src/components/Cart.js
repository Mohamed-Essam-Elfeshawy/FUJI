    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden border border-gray-100">
                {/* Header */}
                <div className="flex items-center justify-between p-4 md:p-6 border-b border-gray-200 bg-gradient-to-r from-fuji-surface to-white">
                    <div className="flex items-center gap-3">
                        ...
                        <h2 className={`text-xl md:text-2xl font-bold text-fuji-blue ${isRTL ? 'font-cairo' : ''}`}>
                            {isRTL ? 'سلة التسوق' : 'Shopping Cart'}
                        </h2>
                    </div>
                    <button
                        onClick={onClose}
                        className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-gray-100 hover:bg-red-100 hover:text-red-600 flex items-center justify-center transition-all duration-200 group"
                    >
                        <svg className="w-5 h-5 md:w-6 md:h-6 text-gray-600 group-hover:text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="flex flex-col h-full max-h-[calc(90vh-80px)]">
                    {items.length === 0 ? (
                        <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-8">
                            <div className="w-20 h-20 md:w-24 md:h-24 bg-fuji-surface rounded-full flex items-center justify-center mb-4">
                                <svg className="w-10 h-10 md:w-12 md:h-12 text-fuji-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4m0 0L7 13m0 0l-2.5 5M7 13l2.5 5m6-5v6a2 2 0 01-2 2H9a2 2 0 01-2-2v-6m8 0V9a2 2 0 00-2-2H9a2 2 0 00-2 2v4.01" />
                                </svg>
                            </div>
                            <h3 className={`text-lg md:text-xl font-semibold text-fuji-blue mb-2 ${isRTL ? 'font-cairo' : ''}`}>
                        ...
                        <>
                            ...
                            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-3 md:space-y-4">
                                {items.map((item) => (
                                    <div key={item.id} className="w-full max-w-full sm:max-w-md md:max-w-lg lg:max-w-xl xl:max-w-2xl 2xl:max-w-3xl mx-auto min-w-0 flex flex-col md:flex-row items-center md:items-stretch gap-3 md:gap-4 p-4 md:p-5 bg-gradient-to-r from-fuji-surface to-white rounded-2xl border border-gray-100 hover:shadow-md transition-all duration-200">
                                        <div className="relative">
                                            ...
                                            />
                                            <div className="absolute -top-2 -right-2 w-5 h-5 md:w-6 md:h-6 bg-fuji-accent text-white rounded-full flex items-center justify-center text-[10px] md:text-xs font-bold">
                                                {item.qty}
                                            </div>
                                        </div>
                                        <div className="flex-1 w-full">
                                            <h4 className={`font-semibold text-sm md:text-base text-fuji-blue mb-1 ${isRTL ? 'font-cairo' : ''}`}>
                                            ...
                                            <p className={`text-fuji-accent font-bold flex items-center gap-1 leading-none ${isRTL ? 'font-cairo justify-end' : 'justify-start'}`}>
                                                ...
                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 382 428" className="w-4 h-4 md:w-5 md:h-5 translate-y-[1px]">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <button
                                                ...
                                                className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-white border border-gray-300 hover:bg-gray-50 flex items-center justify-center text-fuji-blue font-semibold"
                                            >
                                                -
                                            </button>
                                            <span className={`w-10 md:w-12 text-center font-semibold text-fuji-blue ${isRTL ? 'font-cairo' : ''}`}>
                                                {item.qty}
                                            </span>
                                            <button
                                                onClick={() => updateQty(item.id, item.qty + 1)}
                                        ...
                                        <button
                                            ...
                                            className="w-6 h-6 md:w-8 md:h-8 rounded-lg bg-red-100 hover:bg-red-200 flex items-center justify-center text-red-600 transition-colors"
                                        >
                            ...
                            {/* Order Summary */}
                            <div className="border-t border-gray-200 p-4 md:p-6 bg-gradient-to-r from-gray-50 to-fuji-surface">
                                <div className="space-y-2 mb-4">
                                    <div className={`flex justify-between text-sm md:text-base text-fuji-muted ${isRTL ? 'font-cairo' : ''}`}>
                                    ...
                                    <div className={`flex justify-between text-base md:text-lg font-bold text-fuji-blue border-t pt-2 ${isRTL ? 'font-cairo' : ''}`}>
                                        <span>{isRTL ? 'الإجمالي:' : 'Total:'}</span>
                                        <span className="flex items-center gap-1 leading-none">
                                            {totals.total.toLocaleString()}
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 382 428" className="w-5 h-5 md:w-6 md:h-6 translate-y-[1px]">
                                              <path d="M223.564 62.2622C239.654 44.2002 249.543 36.0911 268.965 25.839V326.607L223.564 335.988V62.2622Z" fill="rgb(20,111,182)"/>
                                              <path d="M368.627 213.474C378.039 193.607 379.147 184.777 381.667 164.396L33.6534 239.963C25.4006 258.346 22.7442 268.622 21.4724 286.872L368.627 213.474Z" fill="rgb(20,111,182)"/>
                                              <path d="M368.627 305.598C378.039 285.73 379.147 276.9 381.667 256.52L225.225 289.632C224.118 307.843 225.389 317.186 224.118 335.436L368.627 305.598Z" fill="rgb(20,111,182)"/>
                                              <path d="M368.627 397.708C378.039 377.841 379.147 369.011 381.667 348.63L239.067 379.586C231.869 389.519 227.44 406.075 224.118 427.547L368.627 397.708Z" fill="rgb(20,111,182)"/>
                                ...
                                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                                    <button
                                        onClick={clear}
                                        className={`w-full sm:w-28 md:w-32 py-2.5 md:py-4 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-xl font-semibold text-sm md:text-base transition-all duration-200 flex items-center justify-center gap-2 shadow-lg hover:shadow-xl transform hover:scale-105 ${isRTL ? 'font-cairo' : ''}`}
                                    >
                                    ...
                                    <button
                                        ...
                                        disabled={isOrdering}
                                        className={`w-full sm:flex-1 py-2.5 md:py-4 bg-[#25D366] text-white rounded-xl font-bold text-sm md:text-lg transition-all transform hover:scale-105 disabled:opacity-50 disabled:transform-none flex items-center justify-center gap-2 shadow-lg hover:shadow-xl hover:bg-[#20bd5b] ${isRTL ? 'font-cairo' : ''}`}
                                    >
                                        {isOrdering ? (
                                            <>
